...
class NSSFOperatorCharm(CharmBase):
    ...
    def _on_nrf_available(self, event: NRFAvailableEvent) -> None:
        ...
        self._on_nssf_pebble_ready(event)

    def _write_config_file(self, nrf_url: str) -> None:
        jinja2_environment = Environment(loader=FileSystemLoader("src/templates/"))
        template = jinja2_environment.get_template("nssfcfg.conf.j2")
        content = template.render(
            nrf_url=nrf_url,
            nssf_hostname=self._nssf_hostname,
        )
    ...
    def _on_nssf_pebble_ready(self, event: Union[PebbleReadyEvent, NRFAvailableEvent]) -> None:
        ...
        if not self._config_file_is_written:
            ...
            return
        self._container.add_layer("nssf", self._pebble_layer, combine=True)
    ...
    def _pebble_layer(self) -> Layer:
        ...
        return Layer(
            {
                "summary": "nssf layer",
                "description": "pebble config layer for nssf",
                "services": {
                    "nssf": {
                        "override": "replace",
                        "startup": "enabled",
                        "command": f"./nssf --nssfcfg {BASE_CONFIG_PATH}/{CONFIG_FILE_NAME}",
